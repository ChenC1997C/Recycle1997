...
// }
function goOrder() {
    $.get("http://localhost:8080/Recycle/price/check",function(data) {
		if(data!=0){
			window.location.href="http://localhost:8080/Recycle/html/cart.html";
		}else{
			alert("请先登录")
		}
    });
...
function new_addcart(url,act,key){
	...
	$.post(url,{'gid':gid,'key':key,'act':act,'typeid':typeid},function(data){
		...
		if(act	==	'sub'){
			if(obj[0]	==	1){
				gourl			=	url.replace("price/sub","order/cart");;
				//location.href	=	gourl;
				...
				$.post(check_url,{},function(data){
					...
					if(obj[0]	==	0){
						...
						gourl			=	url.replace("ajax/new_addcart","html/cart");
						location.href	=	gourl;
						return;
					}
				})
